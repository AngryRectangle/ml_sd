...
|-----------|---------|---------------|
| Прогноз | MAPE | В день по каждому SKU, агрегируется |
| Прогноз | RMSE | В день по каждому SKU, агрегируется |
...
1. Оценка сложности на бейзлайне:
    Цель бейзлайна: простая модель на 100 SKU × 5 магазинов × 90 дней = ~45 тыс. временных рядов.
    Вычислительная нагрузка:

    - на 1 SKU ≈ 1–3 секунды (на CPU),
...
3. Меры по снижению нагрузки:
    ...
    Параллельная обработка по магазинам и SKU;
    Кеширование инвариантных признаков;
    Установка таймаута прогноза на 1 SKU — 10 секунд.
